...
# written by user SecretAgentMan.
...
def getGraphs(size):
    """Returns a list of all unlabelled graphs on 'size' vertices."""
    out = []
    
    file_contents = open("./simple graphs/graph{N}.g6".format(N=size), "r")
...
def overGraphs(size):
    """Generates unlabelled graphs on 'size' vertices."""
    file_contents = open("./simple graphs/graph{N}.g6".format(N=size), "r")
    lines = file_contents.readlines()
